...
class Encoder(Model):
    ...
    Modifikasi Enhanced:
    - BatchNormalization ditambahkan setelah setiap hidden layer untuk
      menstabilkan distribusi aktivasi antar layer, mempercepat konvergensi,
      dan mengurangi sensitivitas terhadap inisialisasi bobot.
    """
    def __init__(self, hidden_dims=[512, 256], latent_dim=50, dropout_rate=0.5, **kwargs):
        ...
        # Setiap Dense layer diikuti BatchNormalization untuk stabilisasi training
        self.hidden_layers = [Dense(dim, activation='relu') for dim in hidden_dims]
        self.batch_norms   = [BatchNormalization() for _ in hidden_dims]
...
class Decoder(Model):
    ...
    Modifikasi Enhanced:
    - BatchNormalization ditambahkan setelah setiap hidden layer,
      simetris dengan Encoder untuk konsistensi arsitektur.
    """
...
class VAE(Model):
    ...
    Modifikasi Enhanced:
    - Denoising VAE: input rating di-dropout secara acak saat training agar
      model belajar representasi yang lebih robust (tidak overfit ke rating tertentu).
    - Cyclical KL Annealing: beta naik-turun secara periodik untuk mencegah
      posterior collapse lebih efektif dari linear annealing.
    - BatchNormalization: sudah diterapkan di Encoder dan Decoder.
    """
    def __init__(self, encoder, decoder, beta=1.0, input_dropout_rate=0.5, **kwargs):
        super(VAE, self).__init__(**kwargs)
        self.encoder = encoder
        self.decoder = decoder

        # Simpan beta sebagai tf.Variable agar nilainya bisa diperbarui secara
        # dinamis oleh CyclicalKLAnnealingCallback di setiap epoch tanpa perlu rebuild model
        self.beta = tf.Variable(float(beta), trainable=False, dtype=tf.float32, name='beta')

        self.sampling = Sampling()

        # Denoising VAE: dropout pada input rating saat training
        # Memaksa model belajar representasi robust dari input yang sebagian di-noise
    ...
    def train_step(self, data):
        ...
        Langkah:
        1. Input rating di-dropout (Denoising VAE) — model belajar dari input noisy
        2. Encoder menghasilkan z_mean dan z_log_var (dengan BatchNorm aktif)
        3. Sampling menghasilkan z via reparameterization trick
        4. Decoder menghasilkan output Sigmoid (0-1) (dengan BatchNorm aktif)
        5. Output Sigmoid dinormalisasi per pengguna menjadi distribusi
        6. Reconstruction loss = -sum( x_norm * log(sigmoid_out + eps) ) per pengguna,
           hanya pada item yang ada rating (masking)
        7. KL loss dihitung seperti biasa
        8. Total loss = reconstruction_loss + beta * kl_loss
        """
        if isinstance(data, tuple):
            data = data[0]  # ambil x saja, y dibuang

        with tf.GradientTape() as tape:
            # Denoising VAE: terapkan dropout pada input sebelum masuk encoder
            # Mask asli tetap dipakai untuk menghitung loss (bukan input yang di-noise)
            noisy_input = self.input_dropout(data, training=True)

            # Encoder dan Decoder dipanggil dengan training=True agar BatchNorm aktif
            z_mean, z_log_var = self.encoder(noisy_input, training=True)
            z = self.sampling([z_mean, z_log_var])
            ...
            row_sums    = tf.reduce_sum(masked_data, axis=1, keepdims=True) + 1e-6
            ...
            recon_sums   = tf.reduce_sum(recon_masked, axis=1, keepdims=True) + 1e-6
            ...
            nll_per_item = -tf.reduce_sum(x_norm * tf.math.log(recon_norm + 1e-6) * mask, axis=1)
...
class CyclicalKLAnnealingCallback(tf.keras.callbacks.Callback):
    """
    Callback untuk mengatur beta VAE menggunakan jadwal Cyclical Annealing.

    Berbeda dari linear annealing yang hanya naik sekali, cyclical annealing
    mengulangi siklus naik-turun secara periodik. Setiap siklus terdiri dari:
    - Fase naik  : beta naik linear dari 0 ke beta_target selama ratio*cycle_epochs
    - Fase tetap : beta konstan di beta_target untuk sisa siklus

    Manfaat:
    - Mencegah posterior collapse lebih efektif — model punya kesempatan
      berulang untuk memperbaiki ruang laten di awal setiap siklus baru.
    - Eksplorasi ruang laten lebih baik karena model tidak terjebak di
      satu konfigurasi KL yang konstan.

    Referensi: Fu et al. (2019) "Cyclical Annealing Schedule: A Simple Approach
               to Mitigating KL Vanishing"

    Jadwal annealing (per siklus, misal cycle_epochs=20, ratio=0.5):
        epoch  0 → beta = 0.0          (awal siklus 1, fase naik)
        epoch  9 → beta = beta_target  (akhir fase naik)
        epoch 19 → beta = beta_target  (akhir siklus 1, fase tetap)
        epoch 20 → beta = 0.0          (awal siklus 2, reset)
        ...
    """
    def __init__(self, beta_target, cycle_epochs, ratio=0.5):
        """
        Args:
            beta_target   : nilai beta maksimum yang ingin dicapai
            cycle_epochs  : jumlah epoch per siklus penuh
            ratio         : proporsi siklus untuk fase naik (default 0.5 = 50%)
        """
        super(CyclicalKLAnnealingCallback, self).__init__()
        self.beta_target  = float(beta_target)   if beta_target  is not None else 0.01
        self.cycle_epochs = int(cycle_epochs)     if cycle_epochs is not None else 25
        self.ratio        = float(ratio)          if ratio        is not None else 0.5

    def on_epoch_begin(self, epoch, logs=None):
        """
        Dipanggil otomatis oleh Keras di awal setiap epoch.
        Menghitung posisi dalam siklus dan menetapkan nilai beta.
        """
        if epoch is None:
            return
        # Posisi dalam siklus saat ini (0 sampai cycle_epochs - 1)
        cycle_pos     = epoch % self.cycle_epochs

        # Jumlah epoch untuk fase naik dalam satu siklus
        ramp_epochs   = max(1, int(self.cycle_epochs * self.ratio))

        # Hitung beta: naik linear saat dalam fase naik, konstan setelahnya
        if cycle_pos < ramp_epochs:
            beta_now = (cycle_pos / ramp_epochs) * self.beta_target
        else:
            beta_now = self.beta_target
    ...
    def on_epoch_end(self, epoch, logs=None):
        """
        Tampilkan nilai beta aktif dan posisi siklus untuk monitoring.
        """
        if epoch is None:
            return
        current_beta = float(self.model.beta.numpy())
        cycle_num    = (epoch // self.cycle_epochs) + 1
        cycle_pos    = (epoch % self.cycle_epochs) + 1
        print(f"  [Cyclical KL] Epoch {epoch + 1} | Siklus {cycle_num} pos {cycle_pos}/{self.cycle_epochs} | beta = {current_beta:.6f} / {self.beta_target}")
